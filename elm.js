...
Elm.TodoManager.make = function (_elm) {
   ...
   var LoadTodos = {ctor: "LoadTodos"};
   var ClearTodo = function (a) {
      return {ctor: "ClearTodo",_0: a};
   };
   var EndEditingTodo = function (a) {
      return {ctor: "EndEditingTodo",_0: a};
   };
   var StartEditingTodo = function (a) {
      return {ctor: "StartEditingTodo",_0: a};
   };
   var ClearCompleted = {ctor: "ClearCompleted"};
   var ChangeFilter = function (a) {
   ...
   var footer = F2(function (address,model) {
      var nbActives = $List.length(A2($List.filter,
      function (t) {
         return $Basics.not(t.isCompleted);
      },
      model.todos));
      var nbTotal = $List.length(model.todos);
      var textFinal = _U.eq(nbActives,
      0) ? "Bravo, vs pouvez ne plus rien faire !" : _U.eq(nbActives,
      1) ? "Un dernier effort...il n\'en reste plus qu\'une !" : A2($Basics._op["++"],
      $Basics.toString(nbActives),
      A2($Basics._op["++"],
      "/",
      A2($Basics._op["++"],
      $Basics.toString(nbTotal),
      " taches restantes")));
      return A2($Html.div,
      _U.list([]),
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A3($Html$Lazy.lazy2,
              ...
              model.textInput)
              ,A2($Html.ul,
              _U.list([$Html$Attributes.id("todoList")]),
              A2($List.map,
              function (todo) {
                 return A2($Html.li,
                 _U.list([$Html$Attributes.id($Basics.toString(todo.id))]),
                 _U.list([A2(createTodoItem,address,todo)]));
              },
   ...
   var sampleTodos = _U.list([A3(Todo,"Regarder un film",0,false)
                             ,A3(Todo,"Regarder un film",1,true)]);
   var initModel = {todos: sampleTodos
   ...
   var update = F2(function (action,model) {
      var _p5 = action;
      switch (_p5.ctor)
      {case "LoadTodos": return _U.update(model,{todos: sampleTodos});
         case "ChangeFilter": return _U.update(model,
           {currentFilter: _p5._0});
         case "UpdateInputText": return _U.update(model,
           {textInput: _p5._0});
         case "AddTodo": return addTodo2(model);
         case "UpdateIsTodoCompleted": var _p6 = _p5._0;
           return A2(updateTodo,A3(Todo,_p6.text,_p6.id,_p5._1),model);
...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var main = $StartApp$Simple.start({model: $TodoManager.initModel
